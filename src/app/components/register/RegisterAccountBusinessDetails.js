...
class RegisterAccountBusinessDetails extends Component {
  ...
  onFinish(values, dispatch){
    const data = {};
    data.register = this.props.register;
    data.register.type = values.commercialtype;
    data.register.currency = values.currency;
    console.log(data);
    dispatch(RegisterAction.registerBusinessDetails(values));
    dispatch(ProfileAction.restUpdateProfileDetails(values));
  }
            ...
            <div className="row">
              <div className="card-action col-xs-offset-9 col-xs-3">
                ...
                <RaisedButton
                  ...
                  type='submit'
                  secondary={true}
                   onClick={handleSubmit(this.onFinish)}
                />
              </div>
            </div>
          </div>
